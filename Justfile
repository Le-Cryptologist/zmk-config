...
_parse_combos:
    ...
    if [[ -f $cconf ]]; then
        ...
        for f in "{{ config }}"/*.conf; do
            sed -E "/CONFIG_ZMK_COMBO_MAX_COMBOS_PER_KEY/s/=.+/=$count/" "$f" >"$f.tmp" && mv "$f.tmp" "$f"
        done
        ...
        for f in "{{ config }}"/*.conf; do
            sed -E "/CONFIG_ZMK_COMBO_MAX_KEYS_PER_COMBO/s/=.+/=$count/" "$f" >"$f.tmp" && mv "$f.tmp" "$f"
        done
...
_build_single $board $shield $snippet *west_args:
    ...
    west build -s zmk/app -d "$build_dir" -b $board {{ west_args }} ${snippet:+-S "$snippet"} -- \
        ...
        -DZMK_EXTRA_MODULES="{{ config / 'recorder' }}"
