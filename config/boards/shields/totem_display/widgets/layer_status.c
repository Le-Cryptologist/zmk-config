...
int zmk_widget_layer_status_init(struct zmk_widget_layer_status *widget, lv_obj_t *parent) {
    ...
    lv_obj_set_width(widget->obj, CONFIG_ZMK_TOTEM_DISPLAY_LAYER_NAME_SCROLL_WIDTH);
    lv_label_set_long_mode(widget->obj, LV_LABEL_LONG_SCROLL_CIRCULAR);

    // Set text alignment based on config
    if (strcmp(CONFIG_ZMK_TOTEM_DISPLAY_LAYER_TEXT_ALIGN, "right") == 0) {
        lv_obj_set_style_text_align(widget->obj, LV_TEXT_ALIGN_RIGHT, 0);
    } else if (strcmp(CONFIG_ZMK_TOTEM_DISPLAY_LAYER_TEXT_ALIGN, "center") == 0) {
